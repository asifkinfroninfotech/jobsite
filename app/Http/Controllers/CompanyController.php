...
//                                       'documentstatus' => $documentstatus
        ...
        public function gallerythumbnailUpdate( Request $request) {
        $helper= \App\Helpers\AppHelper::instance();
         if( $request->hasFile('file') )
         {
                if( $request->input('gallerythumbnail')  ) {
                   $response['status'] = 0 ;                
                   
                   $gallery_images = $request->file('file');
                   $galleryid = $request->hiddengalleryid;
                   ...
                   $gallery_name=$helper->fnGetUniqueID('16','gallery','thumbnail');
                   $gallery_name = $gallery_name.'.'.$extension;


                 
                   $gallery_images->move('storage/company/gallery/thumbnail', $gallery_name);
