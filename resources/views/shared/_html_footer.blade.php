...
<script>
    function hidehelp() {
        ...
        $.get('/hidehelpform', function (data) {
            location.reload();
        });
    ...
    function showhelp() {
        //$('#helpform').css('display','none');
        $.get('/showhelpform', function (data) {
            location.reload();
        });
...
<script type="text/javascript">
    ...
    app.controller('gallery', function ($scope, $http) {
        $http.get("{{ route('gallery.list') }}")
            .then(function (response) {
                $scope.galleryimages = response.data.galleryimages;
                $scope.galleryvideos = response.data.galleryvideos;
                //   $scope.privatedoc = response.data.private;
                $scope.statuscode = response.status;
                $scope.statustext = response.statusText;
                $scope.updatethumbnailimage = function (galleryid) {
                    Dropzone.forElement("#uploadgallerythumbnail").removeAllFiles(true);
                    $('#hiddengalleryvideo').val(galleryid);
                    $('#galleryvideothumbnail').modal('show');
