                  ...
                  <tr>
                    ...
                    <td>
                      <div class="user-with-avatar">
                        <a href="/tenant/profile/view?tenid={{$pd->tenantid.'&calledfrom=admin'}}" target="_blank">
                          @if((isset($pd->tenantminilogo) && !empty($pd->tenantminilogo) ) &&
                          File::exists(public_path('/storage/tenant/minilogoimage/'.$pd->tenantminilogo)))
                          <img alt="" src="/storage/tenant/minilogoimage/{{$pd->tenantminilogo}}" />
                          @else
                          <img alt="" src="{{ Avatar::create(ucfirst($pd->tenantcompany))->toBase64() }}" />
                          @endif
                        </a>
                        <span>{{$pd->tenantcompany}}</span>
                      </div>
                    ...
                    <td>
                      <div class="user-with-avatar  ">
                        <a href="/company/profile/view?{{'company='.$pd->companyid .'&companytype='.$pd->companytype}}&calledfrom=admin"
                          target="_blank">
                          @if( (isset($pd->companyprofileimage) && !empty($pd->companyprofileimage) ) &&
                          File::exists(public_path('/storage/company/profileimage/'.$pd->companyprofileimage)))
                          <img alt="" src="/storage/company/profileimage/{{$pd->companyprofileimage}}" />
                    ...
                    <td>
                      ...
                      <div class="btn-group mr-1 mb-1 show">
                        ...
                        <button aria-expanded="false" aria-haspopup="true" class="btn btn-primary dropdown-toggle"
                          data-toggle="dropdown" id="dropdownMenuButton1" type="button">{{trans('adminview.dd_action_btn_label')}}</button>
                        <div aria-labelledby="dropdownMenuButton1" class="dropdown-menu" x-placement="bottom-start"
                          ...
                          <a class="dropdown-item" href="/due-diligence-dashboard?pd={{isset($pd->parentpipelinedealid)==true?$pd->parentpipelinedealid:$pd->pipelinedealid}}&calledfrom=admin&companyid={{$pd->companyid}}&tenantid={{$pd->tenantid}}">{{trans('adminview.dd_view_btn_label')}}</a>
                          <a class="dropdown-item" href="/gotoadmin?companyid={{$pd->companyid}}">{{trans('adminview.company_transfer_btn_label')}}</a></div>
                      </div>
